 ...
 * 지출 항목과 그 **성격**.
 *
 * 성격은 원래 제출 양식에서 받는 값이라 항목 이름으로 추측하지 않는다.
 * 다만 이건 샘플 데이터이므로 처음부터 성격을 함께 적어 둔다 —
 * 성격별 집계 화면이 무엇을 보여주는지 확인할 수 있어야 하기 때문이다.
 */
const EXPENSE_ITEMS = [
  { item: '상품 매입 대금', cat: '매입' },
  { item: '원부자재 매입 정산', cat: '매입' },
  { item: '외주 시공비 정산', cat: '외주' },
  { item: '차입금 이자', cat: '금융' },
  { item: '장비 리스료', cat: '금융' },
  { item: '포장·물류비', cat: '물류' },
  { item: '소모품·공구 구매', cat: '소모품' },
  { item: '보험료 정산', cat: '보험' },
  { item: '세금 납부', cat: '세금' },
  { item: '급여 정산', cat: '인건비' },
  { item: '시설 보수비', cat: '시설' },
...
const oneoff = [];
{
  ...
  while (d <= iso(YEAR, 12, 31) && n < 61) {
    const kind = pick(EXPENSE_ITEMS);
    oneoff.push({
      item: `${kind.item} (${n + 1}차)`,
      cat: kind.cat,
      date: d,
      amt: money(280, 4700),
      corp: pick(CORPS),
